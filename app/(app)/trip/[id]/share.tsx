...
export default function ShareTrip() {
  ...
  const invite = () => {
    ...
    addCollaborator(trip.id, name.trim(), email.trim());
    setName('');
    setEmail('');
  };

  const confirmRemove = (c: Collaborator) =>
    confirmAction('Remove collaborator', `Remove ${c.name} from "${trip.name}"?`, () => removeCollaborator(c.id), { confirmLabel: 'Remove' });
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <Button label="Send invite" icon="add" onPress={invite} disabled={!validEmail} full />
